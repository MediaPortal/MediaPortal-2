...
{
  public class LabelReflectionProperty : ILabelProperty
  {
    #region ILabelProperty Members

    private string _property;

    public LabelReflectionProperty(string property)
    {
      _property = property;
    }
    ...
    public string Evaluate(IControl control, IControl container)
    {
      try
      {
        ...
        object model = ObjectFactory.GetObject(control, window, parts[0]);
        if (model != null)
        {
          int partNr = 1;
          object obj = null;
          while (partNr < parts.Length)
          {
            ...
            MethodInfo info =
              classType.GetProperty(parts[partNr],
                                    ...
                                    BindingFlags.GetProperty | BindingFlags.ExactBinding).GetGetMethod();
