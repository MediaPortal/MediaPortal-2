...
namespace MediaPortal.Plugins.SlimTv.Client.Models
{
  ...
  /// </summary>
  public abstract class SlimTvClientModelBase : SlimTvModelBase
  {
    ...
    public async Task<bool> TuneByIndex(int channelIndex)
    {
      ...
      if (channelIndex >= channels.Count)
        return false;
      await Tune(channels[channelIndex]);
      return true;
    }
    ...
    {
      var channels = GetChannelNavigationList();
      IChannel channel = channels.FirstOrDefault(c => c.ChannelNumber == channelNumber);
      if (channel == null)
        return false;
    ...
    {
      // Special case of this model, which is also used as normal backing model for OSD, where no WorkflowManager action was performed.
      ...
      var channels = GetChannelNavigationList();
      if (channels.MoveTo(c => ChannelContext.IsSameChannel(c, channel)))
        _zapChannelIndex = channels.CurrentIndex; // Needs to be the same to start zapping from current offset
      else
        _zapChannelIndex = 0;
    ...
    protected bool ShouldAutoTune()
    {
      ...
      if (_mediaMode == MediaMode.Tv && !settings.AutoStartTV)
        return false;
      else if (_mediaMode == MediaMode.Radio && !settings.AutoStartRadio)
        return false;
      else if (_mediaMode == MediaMode.All && !settings.AutoStartTV)
        return false;
    ...
    protected async Task AutoTuneLastChannel()
    {
      ...
      GetCurrentChannel();
      var channels = GetChannelNavigationList();
      IChannel current = channels.Current;
      if (current != null)
        await Tune(current);
    }
    ...
    protected override void SetChannel()
    {
      base.SetChannel();
      var channels = GetChannelNavigationList();
      _zapChannelIndex = channels.CurrentIndex; // Use field, as parameter might be changed by base method
    }
    ...
    public async Task ZapNextChannel()
    {
      _zapChannelIndex++;
      var channels = GetChannelNavigationList();
      if (_zapChannelIndex >= channels.Count)
        _zapChannelIndex = 0;
    ...
    public async Task ZapPrevChannel()
    {
      ...
      if (_zapChannelIndex < 0)
        _zapChannelIndex = channels.Count - 1;
    ...
    public async void RecordDialog()
    {
      if (await InitActionsList())
      {
        ...
        if (_mediaMode == MediaMode.Radio)
          screenManager.ShowDialog("DialogClientModelRadio");
        else
    ...
    private async Task ReSetSkipTimer()
    {
      ShowOSD();
      var channels = GetChannelNavigationList();
      await UpdateRunningChannelPrograms(channels[_zapChannelIndex]);
    ...
    private void ZapTimerElapsed(object sender, EventArgs e)
    {
      CloseOSD();
      var channels = GetChannelNavigationList();
      if (!ChannelContext.IsSameChannel(channels[_zapChannelIndex], _lastTunedChannel))
      {
        channels.SetIndex(_zapChannelIndex);
        _ = Tune(channels[_zapChannelIndex]);
      }
    ...
    protected async override void Update()
    {
      ...
      var channels = GetChannelNavigationList();
      _zapChannelIndex = channels.CurrentIndex;
    ...
    protected async Task UpdateChannels()
    {
      ...
      channels.MoveTo(c => ChannelContext.IsSameChannel(c, _lastTunedChannel));
    ...
    protected async Task GetNowAndNextProgramsList_Async()
    {
      ...
      lock (_channelList.SyncRoot)
        foreach (ChannelProgramListItem channelItem in _channelList)
        {
          IProgram[] nowNext;
          IProgram currentProgram = null;
          IProgram nextProgram = null;
