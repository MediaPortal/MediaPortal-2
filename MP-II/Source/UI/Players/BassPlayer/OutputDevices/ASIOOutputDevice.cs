...
namespace Ui.Players.BassPlayer.OutputDevices
{
  ...
  /// </summary>
  internal class ASIOOutputDevice : IOutputDevice
  {
    ...
    protected readonly Dictionary<int, DeviceInfo> _DeviceInfos = new Dictionary<int, DeviceInfo>();
    ...
    private readonly int[] SampleRates = new int[] {
        8000,
        9600,
        11025,
        12000,
    ...
    public void SetInputStream(BassStream stream)
    {
      ...
      _FirstOutputChannel = _Player.Settings.ASIOFirstChan == BassPlayerSettings.Constants.Auto ?
                                                                                                    0 : _Player.Settings.ASIOFirstChan;
      ...
      _LastOutputChannel = _Player.Settings.ASIOLastChan == BassPlayerSettings.Constants.Auto ?
                                                                                                  _Driver.OutputChannels.Length : _Player.Settings.ASIOLastChan;
      ...
      const BASSFlag streamFlags = BASSFlag.BASS_SAMPLE_FLOAT | BASSFlag.BASS_STREAM_DECODE;
      ...
      if (!_Player.InputSourceSwitcher.CurrentInputSource.OutputStream.IsPassThrough)
    ...
    private void Initialize()
    {
      ...
      _STAThread = new Thread(STAThread)
        {
            Name = "ASIOOutputDevice work item executor thread"
        };
      ...
      _Driver.BufferUpdate += _Driver_BufferUpdate;
    ...
    private void CollectDeviceInfo(int deviceNo)
    {
      ...
      if (!_DeviceInfos.ContainsKey(deviceNo))
      {
        ...
        DeviceInfo deviceInfo = new DeviceInfo
          {
              _Name = _Driver.DriverName,
              _Driver = _Driver.Version.ToString(),
              _Channels = _Driver.OutputChannels.Length,
              _MaxRate = GetMaxASIORate(),
              _MinRate = GetMinASIORate(),
              _Latency = GetASIOLatency()
          };
      ...
      Log.Debug("ASIO Device info: {0}", _DeviceInfos[_DeviceNo]);
    }
    ...
    {
      int samplesRead = _MixerStream == null ? 0 : _MixerStream.Read(_Buffer);
      ...
      if (samplesRead == 0)
      {
        ...
        if (!_OutputStreamEnded)
        {
          ...
          _Player.HandleOutputStreamEnded();
        }
      }
      ...
      for (int index = 0; index < _Buffer.Length; index++)
      {
        if (channelIndex <= _LastOutputChannel)
          _Driver.OutputChannels[channelIndex][channelSample] = index < samplesRead ? _Buffer[index] : 0.0f;
