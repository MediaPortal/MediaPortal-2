...
float4 PixelEffect(in float2 texcoord, in Texture2D InputTexture, in sampler TextureSampler, in float4 framedata) : COLOR
{
  ...
  ori = InputTexture.Sample(TextureSampler, texcoord);
  float4 c1 = InputTexture.Sample(TextureSampler, texcoord + float2(-dx, -dy));
  float4 c2 = InputTexture.Sample(TextureSampler, texcoord + float2(0, -dy));
  float4 c3 = InputTexture.Sample(TextureSampler, texcoord + float2(dx, -dy));
  float4 c4 = InputTexture.Sample(TextureSampler, texcoord + float2(-dx, 0));
  float4 c5 = InputTexture.Sample(TextureSampler, texcoord + float2(dx, 0));
  float4 c6 = InputTexture.Sample(TextureSampler, texcoord + float2(-dx, dy));
  float4 c7 = InputTexture.Sample(TextureSampler, texcoord + float2(0, dy));
  float4 c8 = InputTexture.Sample(TextureSampler, texcoord + float2(dx, dy));
