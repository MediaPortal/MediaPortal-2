...
namespace MediaPortal.Plugins.SlimTv.Client.Models
{
  public abstract class SlimTvScheduleManagementBase : SlimTvModelBase
  {
    ...
    protected async Task LoadSchedules()
    {
      ...
      //TODO: Find more efficient way of determining whether a schedule is for tv or radio
      var channelIds = result.Result.Where(s => s.ChannelId > 0).Select(s => s.ChannelId).Distinct().ToList();
      Dictionary<int, IChannel> channels = new Dictionary<int, IChannel>();
      if (channelIds.Any())
      {
        var groups = ChannelContext.Instance.TvChannelGroups.Concat(ChannelContext.Instance.RadioChannelGroups).ToList();
        var currentTvGroupId = ChannelContext.Instance.TvChannelGroups.Current?.ChannelGroupId ?? 0;
        var currentRadioGroupId = ChannelContext.Instance.RadioChannelGroups.Current?.ChannelGroupId ?? 0;
        channels = ChannelContext.Instance.TvChannels.Concat(ChannelContext.Instance.RadioChannels).ToDictionary(c => c.ChannelId);

        if (!channelIds.All(i => channels.ContainsKey(i)))
        {
          foreach (var group in groups)
          {
            if (group.ChannelGroupId == currentTvGroupId || group.ChannelGroupId == currentRadioGroupId)
              continue;

            var channelsResult = await _tvHandler.ChannelAndGroupInfo.GetChannelsAsync(group);
      ...
      if (_mediaMode == MediaMode.Tv)
        schedules = result.Result.Where(s => s.ChannelId == 0 || (channels.ContainsKey(s.ChannelId) && channels[s.ChannelId].MediaType == MediaType.TV)).ToList();
      else if (_mediaMode == MediaMode.Radio)
        schedules = result.Result.Where(s => s.ChannelId == 0 || (channels.ContainsKey(s.ChannelId) && channels[s.ChannelId].MediaType == MediaType.Radio)).ToList();
    ...
    {
      string title = scheduleName.Replace(UPnP.Consts.MANUAL_RECORDING_TITLE_PREFIX, "");
