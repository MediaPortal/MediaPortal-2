...
namespace MediaPortal.Plugins.SlimTv.Client.Models
{
  ...
  /// </summary>
  public abstract class SlimTvMultiChannelGuideModelBase : SlimTvGuideModelBase
  {
    ...
    private void ShowSingleChannelGuide(IChannel channel)
    {
      ...
      if (_mediaMode == MediaMode.Tv)
        stateId = new Guid("45235AE7-0329-483F-A1B1-945DF0D998CE");
      else if (_mediaMode == MediaMode.Radio)
        stateId = new Guid("7365DA33-4687-43F4-9652-F6652468D4B8");
    ...
    private async Task InternalUpdatePrograms()
    {
    ...
    protected async Task UpdateProgramsForGroup()
    {
      ...
      if (
        _bufferGroupIndex != groups.CurrentIndex || /* Group changed */
      ...
      {
        _bufferGroupIndex = groups.CurrentIndex;
    ...
    public override void Reactivate(NavigationContext oldContext, NavigationContext newContext)
    {
      ...
      var groups = GetGroupNavigationList();
      if (timeChanged || _bufferGroupIndex != groups.CurrentIndex)
      {
        UpdateChannels();
        _ = UpdatePrograms();
      }
