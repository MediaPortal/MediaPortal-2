...
namespace MediaPortal.Core.UPnP
{
  ...
  public class UPnPDtPlaylistRawData : UPnPExtendedDataType
  {
    ...
    protected override void DoSerializeValue(object value, bool forceSimpleValue, XmlWriter writer)
    {
      ...
      foreach (PlaylistRawData metadata in clientData)
        metadata.Serialize(writer);
    }

    protected override object DoDeserializeValue(XmlReader reader, bool isSimpleValue)
    {
      ICollection<PlaylistRawData> result = new List<PlaylistRawData>();
      if (SoapHelper.ReadEmptyStartElement(reader)) // Read start of enclosing element
        return result;
      while (reader.NodeType != XmlNodeType.EndElement)
        result.Add(PlaylistRawData.Deserialize(reader));
