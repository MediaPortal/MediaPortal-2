...
float4 PixelEffect(in float2 texcoord, in Texture2D InputTexture, in sampler TextureSampler, in float4 framedata) : COLOR
{
  ...
  float4 cAccum = InputTexture.Sample(TextureSampler, texcoord) * CenterBias;
  ...
  for (int iDx = 0; iDx < 16; ++iDx)
  {
    ...
    cAccum += InputTexture.Sample(TextureSampler, texcoord + float2(-dx, -dy)) * SampleBias;
    cAccum += InputTexture.Sample(TextureSampler, texcoord + float2(0, -dy)) * SampleBias;
    cAccum += InputTexture.Sample(TextureSampler, texcoord + float2(-dx, 0)) * SampleBias;
    cAccum += InputTexture.Sample(TextureSampler, texcoord + float2(dx, 0)) * SampleBias;
    cAccum += InputTexture.Sample(TextureSampler, texcoord + float2(0, dy)) * SampleBias;
    cAccum += InputTexture.Sample(TextureSampler, texcoord + float2(dx, dy)) * SampleBias;
    cAccum += InputTexture.Sample(TextureSampler, texcoord + float2(-dx, +dy)) * SampleBias;
