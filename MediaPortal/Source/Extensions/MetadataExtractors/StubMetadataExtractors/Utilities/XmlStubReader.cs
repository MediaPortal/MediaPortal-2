...
namespace MediaPortal.Extensions.MetadataExtractors.StubMetadataExtractors.Utilities
{
  /// <summary>
  /// <see cref="XmlReader"/> specifically designed to read stub-files
  /// </summary>
  internal sealed class XmlStubReader : XmlWrappingReader
  {
    // Stub-files sometimes contain more than one root element, which according to xml-specs
    // results in a not "well-formed" xml-file. The XmlStubReader transparently wrappes everything
    // after the xml-prolog of such stub-files into a single <root> element.
    ...
    public XmlStubReader(Stream xmlStream) : base(Create(xmlStream, new XmlReaderSettings { ConformanceLevel = ConformanceLevel.Fragment }))
    {
    }
