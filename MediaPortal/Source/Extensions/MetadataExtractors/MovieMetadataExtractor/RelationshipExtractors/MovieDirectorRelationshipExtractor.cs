...
{
  class MovieDirectorRelationshipExtractor : IRelationshipRoleExtractor, IMovieRelationshipExtractor
  {
    ...
    public bool TryExtractRelationships(IDictionary<Guid, IList<MediaItemAspect>> aspects, out ICollection<IDictionary<Guid, IList<MediaItemAspect>>> extractedLinkedAspects, bool forceQuickMode)
    {
      ...
      if (!MovieMetadataExtractor.SkipOnlineSearches)
        OnlineMatcherService.Instance.UpdatePersons(movieInfo, PersonAspect.OCCUPATION_DIRECTOR, forceQuickMode);

      if (movieInfo.Directors.Count == 0)
        return false;

      if (BaseInfo.CountRelationships(aspects, LinkedRole) < movieInfo.Directors.Count)
