...
namespace MediaPortal.Plugins.SlimTv.Interfaces.UPnP.DataTypes
{
  public class UPnPDtScheduleRuleTargetList : UPnPExtendedDataType
  {
    ...
    public UPnPDtScheduleRuleTargetList()
    ...
    {
      IEnumerable targets = (IEnumerable)value;
      foreach (ScheduleRuleTarget target in targets)
        target.Serialize(writer);
    }

    protected override object DoDeserializeValue(XmlReader reader, bool isSimpleValue)
    {
      ICollection<ScheduleRuleTarget> result = new List<ScheduleRuleTarget>();
      if (SoapHelper.ReadEmptyStartElement(reader)) // Read start of enclosing element
        return result;
      while (reader.NodeType != XmlNodeType.EndElement)
        result.Add(ScheduleRuleTarget.Deserialize(reader));
