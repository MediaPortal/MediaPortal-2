...
{
  /// <summary>
  /// <see cref="SlimTvProgramSearchModelBase"/> holds all data for extended scheduling options.
  /// </summary>
  public abstract class SlimTvProgramSearchModelBase : SlimTvGuideModelBase
  {
    #region Fields
    ...
    {
      _programsList.Clear();
      var channels = GetChannelNavigationList().ToDictionary(c => c.ChannelId);
      foreach (IProgram program in _programs)
      {
        if (!channels.ContainsKey(program.ChannelId))
          continue;
        if (_mediaMode == MediaMode.Tv && channels[program.ChannelId].MediaType != MediaType.TV)
          continue;
        if (_mediaMode == MediaMode.Radio && channels[program.ChannelId].MediaType != MediaType.Radio)
          continue;
