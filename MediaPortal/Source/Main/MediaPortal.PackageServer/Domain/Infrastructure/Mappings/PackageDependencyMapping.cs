...
{
	internal class PackageDependencyMapping : AbstractMappingProvider<PackageDependency>
	{
		public override void DefineModel( DbModelBuilder modelBuilder )
		{
			base.DefineModel( modelBuilder );

		  Map.HasKey( e => new { e.ReleaseID, e.PackageID } );

			Map.Property( e => e.MinCompatibleApi ).IsRequired();
			Map.Property( e => e.MaxCompatibleApi ).IsRequired();

		  Map.HasRequired( e => e.Release ).WithMany().HasForeignKey( e => e.ReleaseID );
		  Map.HasRequired( e => e.Package ).WithMany().HasForeignKey( e => e.PackageID );
		}
	}
}