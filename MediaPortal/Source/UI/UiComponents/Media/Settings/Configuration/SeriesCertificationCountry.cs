...
{
  public class SeriesCertificationCountry : SingleSelectionList
  {
    ...
    public SeriesCertificationCountry()
    {
      List<RegionInfo> regions = CertificationMapper.GetSupportedSeriesCertificationCountries().Select(r => new RegionInfo(r)).ToList();
    ...
    public override void Save()
    {
      ...
      MediaCertificationSettings settings = SettingsManager.Load<MediaCertificationSettings>();
      settings.DisplaySeriesCertificationCountry = "";
      if (Selected > 0)
      {
        settings.DisplaySeriesCertificationCountry = _regions[Selected - 1].Name;
      }
      SettingsManager.Save(settings);
      CertificationHelper.DisplayMovieCertificationCountry = settings.DisplaySeriesCertificationCountry;
    }
  }
}
