...
namespace MediaPortal.PackageManager.Core.Package
{
  ...
  public class PackageReleaseMetaData
  {
    ...
    public PackageReleaseMetaData(XElement xRoot)
    {
      ...
      UninstallActions = new PackageActionCollection();
      ...
      UninstallActions = ParseActions(xRoot, PackageInstallType.Uninstall);
    ...
    /// <summary>
    /// Gets the collection with all uninstall actions.
    /// </summary>
    public PackageActionCollection UninstallActions { get; private set; }
    ...
    public bool CheckValid(PackageRoot packageRoot, out string message)
    {
      ...
      foreach (var action in UninstallActions)
      {
        if (!action.CheckValid(packageRoot, out message))
        {
          message = String.Format("Uninstall action {0}: {1}", action.Description ?? action.GetType().Name, message);
          return false;
        }
      }
      message = null;
    ...
    public static PackageActionCollection ParseActions(XElement xRoot, PackageInstallType installType)
    {
      ...
      switch (installType)
      {
        ...
        case PackageInstallType.Uninstall:
          xActions = xRoot.Element("UninstallActions") ?? new XElement("UninstallActions", new XElement("Action", new XAttribute("Type", "DeletePackageDirectories")));
          break;
