...
namespace MediaPortal.Extensions.MetadataExtractors.SeriesMetadataExtractor
{
  class EpisodeWriterRelationshipExtractor : IRelationshipRoleExtractor
  {
    ...
    public bool TryExtractRelationships(IDictionary<Guid, IList<MediaItemAspect>> aspects, out ICollection<IDictionary<Guid, IList<MediaItemAspect>>> extractedLinkedAspects, bool forceQuickMode)
    {
      ...
      IEnumerable<string> writers = videoAspect.GetCollectionAttribute<string>(VideoAspect.ATTR_WRITERS);

      // Build the person MI

      List<PersonInfo> persons = new List<PersonInfo>();
      if (writers != null) foreach (string person in writers) persons.Add(new PersonInfo() { Name = person, Occupation = PersonOccupation.Writer });

      EpisodeInfo episodeInfo;
      if (!SeriesBaseTryExtractRelationships.GetBaseInfo(aspects, out episodeInfo))
        return false;

      SeriesTheMovieDbMatcher.Instance.UpdateEpisodePersons(episodeInfo, persons, PersonOccupation.Writer);
      SeriesTvMazeMatcher.Instance.UpdateEpisodePersons(episodeInfo, persons, PersonOccupation.Writer);
      SeriesTvDbMatcher.Instance.UpdateEpisodePersons(episodeInfo, persons, PersonOccupation.Writer);
    ...
    {
      return MediaItemAspect.TryGetAttribute(aspects, VideoAspect.ATTR_WRITERS, out index);
    }
