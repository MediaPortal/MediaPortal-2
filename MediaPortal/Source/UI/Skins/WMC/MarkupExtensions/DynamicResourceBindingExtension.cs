...
// Define DEBUG_DRME to log activities of the DynamicResourceBindingMarkupExtension. Setting that switch will slow down the
...
namespace MediaPortal.UiComponents.WMCSkin.MarkupExtensions
{
  /// <summary>
  /// Custom implementation of <see cref="DynamicResourceExtension"/> that additionally supports
  /// binding to ResourceKey to allow dynamically selecting.resources with different keys.
  /// <seealso cref="TreeSearchMode"/>
  /// </summary>
  /// <remarks>
  /// This class is almost identical to <see cref="DynamicResourceExtension"/> except that
  /// it allows binding to the ResourceKey, the ResourceKey is allowed to be null
  /// and it falls back to SkinResources when looking up resources if a matching resources was not
  /// found in the tree.
  ...
  /// </remarks>
  public class DynamicResourceBindingExtension : BindingBase
  {
    ...
    public DynamicResourceBindingExtension(string resourceKey): this()
    {
    ...
    protected bool UpdateTarget()
    {
      ...
      while (current != null)
      {
        ...
        if (!FindParent(current, out current))
          //Fall back to skin resources to allow the ResourceKey
          //to point to a Static/ThemeResource
          break;
      }
    ...
    public override void Activate()
    {
      if (_active)
        return;
      base.Activate();
      //ResourceKey may be bound to a property that is not yet available
      //only update here if the key is known
      if (ResourceKey != null)
        UpdateTarget();
    }
    ...
    {
      return "{DynamicResourceBinding " + ResourceKey + "}";
    }
