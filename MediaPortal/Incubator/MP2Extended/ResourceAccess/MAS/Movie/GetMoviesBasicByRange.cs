...
namespace MediaPortal.Plugins.MP2Extended.ResourceAccess.MAS.Movie
{
  ...
  {
    public IList<WebMovieBasic> Process(int start, int end, string filter, WebSortField? sort, WebSortOrder? order)
    {
      IEnumerable<WebMovieBasic> output = Process(filter, sort, order);
