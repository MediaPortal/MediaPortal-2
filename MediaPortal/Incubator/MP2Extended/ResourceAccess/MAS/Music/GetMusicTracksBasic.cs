...
namespace MediaPortal.Plugins.MP2Extended.ResourceAccess.MAS.Music
{
  ...
  {
    public IList<WebMusicTrackBasic> Process(string filter, WebSortField? sort, WebSortOrder? order)
    {
      ...
      var output = items.Select(item => MusicTrackBasic(item, trackToArtistMap, trackToAlbumArtistMap))
        .Filter(filter);
