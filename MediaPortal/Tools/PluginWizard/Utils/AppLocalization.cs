...
{
	/// <summary>
	/// Description of AppLocalization.
	/// </summary>
	public class AppLocalization: ILocalization
	{
    ...
    protected object _syncObj = new object();
		...
		public AppLocalization()
		{
			...
			ServiceRegistration.Get<ILogger>().Debug("AppLocalization: Loading settings");
      var settings = ServiceRegistration.Get<ISettingsManager>().Load<RegionSettings>();
      if (string.IsNullOrEmpty(settings.Culture))
      {
        _currentCulture = CultureInfo.CurrentUICulture;
        ServiceRegistration.Get<ILogger>().Info("AppLocalization: Culture not set. Using culture: '{0}'", _currentCulture.Name);
      }
      else
      {
        _currentCulture = CultureInfo.GetCultureInfo(settings.Culture);
        ServiceRegistration.Get<ILogger>().Info("AppLocalization: Using culture: " + _currentCulture.Name);
      }
		}
    ...
    public bool TryTranslate(string section, string name, out string translation, params object[] parameters)
    {
      ...
      {
        ServiceRegistration.Get<ILogger>().Error("AppLocalization: Error formatting localized string '{0}' (Section='{1}', Name='{2}')", e, translation, section, name);
        return false;
      }
      return true;
    }
