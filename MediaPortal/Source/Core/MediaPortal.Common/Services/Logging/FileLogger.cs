...
{
  /// <summary>
  /// A <see cref="Common.Logging.ILogger"/> implementation that writes messages to a text file.
  /// </summary>
  public class FileLogger : DefaultLogger
  {
    ...
    public static void DeleteLogFiles(string logFilePath, string searchPattern)
    {
      ...
      foreach (string logFile in logFiles)
      {
        ...
        {
          Console.WriteLine("FileLogger: Unable to delete old logfile '{0}' (Reason: '{1}')", logFile, e.Message);
        }
        catch (Exception e)
        {
          if (!(e is IOException))
