...
namespace MediaPortal.Extensions.OnlineLibraries.Matches
{
  ...
  /// </summary>
  /// <typeparam name="TMatch">Type of match, must be derived from <see cref="BaseMediaMatch{T}"/>.</typeparam>
  /// <typeparam name="TId">Type of internal ID of the match.</typeparam>
  public abstract class BaseMediaMatcher<TMatch, TId, TImg, TLang> : IDisposable
    where TMatch : BaseMediaMatch<TId>
  {
    #region Constants
