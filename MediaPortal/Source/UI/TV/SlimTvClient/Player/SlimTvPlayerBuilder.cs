  ...
  public class SlimTvPlayerBuilder : IPlayerBuilder
  {
    ...
    public IPlayer GetPlayer(MediaItem mediaItem)
    {
      ...
      if (mimeType != LiveTvMediaItem.MIME_TYPE_TV && mimeType != LiveTvMediaItem.MIME_TYPE_RADIO && mimeType != LiveTvMediaItem.MIME_TYPE_WTVREC &&
          mimeType != LiveTvMediaItem.MIME_TYPE_TV_STREAM && mimeType != LiveTvMediaItem.MIME_TYPE_RECORDED_TV_STREAM)
        return null;
      IResourceLocator locator = mediaItem.GetResourceLocator();
      BaseDXPlayer player =
        mimeType == LiveTvMediaItem.MIME_TYPE_TV ? (BaseDXPlayer)new LiveTvPlayer() :
        mimeType == LiveTvMediaItem.MIME_TYPE_TV_STREAM ? (BaseDXPlayer)new LiveTvStreamPlayer() :
        mimeType == LiveTvMediaItem.MIME_TYPE_RECORDED_TV_STREAM ? (BaseDXPlayer)new TvStreamPlayer() :
