...
{
  public class UPnPDtScheduleRuleList : UPnPExtendedDataType
  {
    public static UPnPDtScheduleRuleList Instance = new UPnPDtScheduleRuleList();

    public const string DATATYPE_NAME = "UPnPDtScheduleRuleList";

    public UPnPDtScheduleRuleList()
    ...
    {
      IEnumerable rules = (IEnumerable)value;
    ...
    {
      ICollection<ScheduleRule> result = new List<ScheduleRule>();
      if (SoapHelper.ReadEmptyStartElement(reader)) // Read start of enclosing element
        return result;
      while (reader.NodeType != XmlNodeType.EndElement)
        result.Add(ScheduleRule.Deserialize(reader));
