...
namespace MediaPortal.Common.PluginManager.Activation
{
  ...
  ///     <list type="bullet">
  ///       <item>
  /// Storing plugin runtime data like the <see cref="State"/>, the <see cref="StateTracker"/>
  /// and the loaded assemblies.
  ///       </item>
  ///       <item>Instantiation of classes stored in assemblies of this plugin.</item>
  ///       <item>
  /// Registration of items in the plugin tree/registry and managing item location change listeners.
  ...
  /// which is shared between all plugins.
  ///       </item>
  ///     </list>
  ///   </para>
  /// </remarks>
  public class PluginRuntime
  {
    ...
    private IPluginStateTracker _stateTracker;
    private ICollection<Assembly> _loadedAssemblies; // Lazy initialized
    private IDictionary<string, ObjectReference> _instantiatedObjects; // Lazy initialized
    ...
    /// <param name="builderManager">
    /// The builder manager where this plugin can register and find
    /// builder instances.
    /// </param>
    ...
    {
      get { return (PluginState)Interlocked.Read(ref _state); }
      private set
      {
        lock (_syncObj)
        {
