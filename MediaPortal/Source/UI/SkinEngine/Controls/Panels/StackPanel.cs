...
namespace MediaPortal.UI.SkinEngine.Controls.Panels
{
  public class StackPanel : Panel, IScrollViewerFocusSupport, IScrollInfo
  {
    ...
    protected virtual void BringIntoView(UIElement element, ref RawRectangleF elementBounds)
    {
      if (_doScroll)
      {
        IList<FrameworkElement> visibleChildren = GetVisibleChildren();
    ...
    public virtual void AlignedPanelAddPotentialFocusNeighbors(RawRectangleF? startingRect, ICollection<FrameworkElement> outElements, bool elementsBeforeAndAfter)
    {
      if (!IsVisible)
        return;
      if (Focusable)
