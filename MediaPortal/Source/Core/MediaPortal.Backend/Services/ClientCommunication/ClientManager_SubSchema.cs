...
namespace MediaPortal.Backend.Services.ClientCommunication
{
  ...
  public class ClientManager_SubSchema
  {
    ...
    public static IDbCommand SelectAttachedClientsCommand(ITransaction transaction, out int systemIdIndex,
        out int lastHostNameIndex, out int lastClientNameIndex, out int lastClientVersionIndex)
    {
      IDbCommand result = transaction.CreateCommand();

      result.CommandText = "SELECT SYSTEM_ID, LAST_HOSTNAME, LAST_CLIENT_NAME, LAST_CLIENT_VERSION FROM ATTACHED_CLIENTS";

      systemIdIndex = 0;
      lastHostNameIndex = 1;
      lastClientNameIndex = 2;
      lastClientVersionIndex = 3;
      return result;
    }
    ...
    {
      IDbCommand result = transaction.CreateCommand();
      result.CommandText = "INSERT INTO ATTACHED_CLIENTS (SYSTEM_ID, LAST_HOSTNAME, LAST_CLIENT_NAME, LAST_CLIENT_VERSION) VALUES (@SYSTEM_ID, @LAST_HOSTNAME, @LAST_CLIENT_NAME, @LAST_CLIENT_VERSION)";
      ...
      database.AddParameter(result, "LAST_CLIENT_VERSION", clientVersion, typeof(string));
      return result;
    }
    ...
    {
      IDbCommand result = transaction.CreateCommand();
      result.CommandText = "UPDATE ATTACHED_CLIENTS SET LAST_HOSTNAME = @LAST_HOSTNAME, LAST_CLIENT_NAME = @LAST_CLIENT_NAME, LAST_CLIENT_VERSION = @LAST_CLIENT_VERSION WHERE SYSTEM_ID = @SYSTEM_ID";
      ISQLDatabase database = transaction.Database;
