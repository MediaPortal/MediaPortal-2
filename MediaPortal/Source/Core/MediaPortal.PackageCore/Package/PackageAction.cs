...
namespace MediaPortal.PackageCore.Package
{
  ...
  public abstract class PackageAction
  {
    ...
    protected virtual void PrintStartAction(PackageActionContext context)
    {
      context.Log.Info(Description ?? String.Format("Executing action {0}", ActionType));
    }
    ...
    protected virtual void PrintFinishAction(PackageActionContext context)
    { }
    ...
    /// </summary>
    /// <param name="context">Execution context.</param>
    /// <remarks>Normally this method is not overridden. Override <see cref="DoExecute(PackageActionContext)"/> instead.</remarks>
    public virtual void Execute(PackageActionContext context)
    {
    ...
    protected abstract void DoExecute(PackageActionContext context);
  }
  ...
  public class PackageActionContext
  {
    ...
    public ILogger Log { get { return PackageRoot.Log; } }

    public PackageActionContext(PackageRoot packageRoot, PackageInstallType installyType, IDictionary<string, string> registredPaths)
    {
  ...
  public abstract class CopyBasePackageAction : PackageAction
  {
    ...
    public static void CopyFile(PackageActionContext context, string source, string target, bool overwrite, bool print)
    {
      if (print)
      {
        context.Log.Info("Copying file {0} to {1}", source, target);
      }
      try
      {
    ...
    /// <param name="print"><c>true</c> if a copy message should be printed to the context output.</param>
    public static void CopyDirectory(PackageActionContext context, string source, string target, bool overwrite, bool topLevelDirectoryCanExist, bool recursive, bool print)
    {
      if (print)
      {
        context.Log.Info("Copying directory {0} to {1}", source, target);
      }
      try
      {
        ...
        foreach (var file in Directory.GetFiles(source))
        {
          ...
          if (print)
          {
            context.Log.Info("Copying file {0}", fileName);
          }
